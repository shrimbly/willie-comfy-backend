...
class ImageGridSplit(IO.ComfyNode):
    ...
    def define_schema(cls):
        return IO.Schema(
            node_id="ImageGridSplit",
            display_name="Image Grid Split",
            category="image/transform",
            search_aliases=["grid", "split image", "slice image", "crop grid"],
            description="Splits an image into a grid of rows x columns tiles, returned as a single batch (row-major).",
            inputs=[
                IO.Image.Input("image"),
